    ...
    <script>
            ...
            const excelUploadForm = document.getElementById('gassExcelUploadForm');
            const excelUploadInput = document.getElementById('gassExcelUploadInput');
            const excelPreviewModalElement = document.getElementById('gassExcelPreviewModal');
            const excelPreviewModal = excelPreviewModalElement ? new bootstrap.Modal(excelPreviewModalElement) : null;
            const excelConfirmImportBtn = document.getElementById('gassExcelConfirmImportBtn');
            const excelPreviewLoading = document.getElementById('gassExcelPreviewLoading');
            const excelPreviewContent = document.getElementById('gassExcelPreviewContent');
            const excelPreviewStats = document.getElementById('gassExcelPreviewStats');
            const excelPreviewWarnings = document.getElementById('gassExcelPreviewWarnings');
            const excelPreviewWarningPanel = document.getElementById('gassExcelPreviewWarningPanel');
            const excelPreviewRows = document.getElementById('gassExcelPreviewRows');
            const excelPreviewSubtitle = document.getElementById('gassExcelPreviewSubtitle');
                    ...
                    if (rows.length === 0) {
                        ...
                        excelPreviewRows.innerHTML = rows.map(row => {
                            const hierarchy = Array.isArray(row?.hierarchy) && row.hierarchy.length > 0
                                ? row.hierarchy.map((item, index) => `<div class="gass-preview-level level-${Math.min(index, 4)}">${escapePreviewHtml(item)}</div>`).join('')
            ...
            if (excelUploadForm && excelUploadInput && excelPreviewModal) {
                excelUploadInput.addEventListener('change', async function () {
                    ...
                    try {
                        ...
                        console.error('GASS Excel preview error:', error);
                        if (excelPreviewSubtitle) {
        ...
        const targetStoreUrl = @json(route('users.gass_physical.targets.store'));
        const accompStoreUrl = @json(route('users.gass_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.gass_physical.rows.destroy'));
        ...
        function getCurrentMonthPendingTotal(row, colIndex) {
            const officeKey = String(currentOfficeId || '').trim();
            if (officeKey) {
                const targetValue = getLivePeriodValue(row, 'target', officeKey, colIndex);
                const accompValue = getLivePeriodValue(row, 'accomp', officeKey, colIndex);
                return targetValue > accompValue ? targetValue - accompValue : 0;
            }
        ...
        function rowHasCurrentMonthPending(row) {
            ...
            if (officeKey) {
                const targetValue = getLivePeriodValue(row, 'target', officeKey, colIndex);
                const accompValue = getLivePeriodValue(row, 'accomp', officeKey, colIndex);
                return targetValue > 0 && targetValue > accompValue;
            }
                ...
                ['target', 'accomp'].forEach(sourceSection => {
                    ...
                    if (pendingCarInput) {
                        pendingCarInput.value = pendingTotal > 0
                            ? getCurrentMonthSectionTotal(row, sourceSection, colIndex)
                            : 0;
                    }
        ...
        function ensurePendingBodyCells() {
            const rows = Array.from(document.querySelectorAll('tbody tr[data-row-id]'));
            if (rows.length === 0) return;

            const hasPendingBodyCells = rows.some(row => row.querySelector('td[data-dynamic-section="pending"]'));
            if (hasPendingBodyCells) return;
            ...
            rows.forEach(row => {
                ...
                ['target', 'accomp'].forEach(sourceSection => {
                    ...
                    td.classList.add('p-1', 'text-center', 'dynamic-cell-pending');
                    td.dataset.dynamicSection = 'pending';
                    ...
                    const appendInput = (officeId, isCarTotal = false) => {
                        const inputLine = document.createElement('div');
                        inputLine.className = 'input-line';

                        const input = document.createElement('input');
                        input.type = 'number';
                        input.className = 'month-box pending-box';
                        input.style.width = '70px';
                        input.style.maxWidth = '150px';
                        input.min = '0';
                        input.step = 'any';
                        input.dataset.section = 'pending';
                        input.dataset.pendingKind = sourceSection;
                        input.dataset.sourceSection = sourceSection;
                        input.dataset.col = String(currentCol);
                        input.dataset.officeId = isCarTotal ? 'car-total' : String(officeId || '');

                        let value = 0;
                        if (isCarTotal) {
                            input.classList.add('car-total-box');
                            input.dataset.carTotal = '1';
                            value = getCurrentMonthSectionTotal(row, sourceSection, currentCol);
                        } else if (officeId) {
                            value = getLivePeriodValue(row, sourceSection, String(officeId), currentCol);
                        }
                        input.value = Number.isFinite(value) ? value : 0;
                    ...
                    appendInput('car-total', true);
                    offices.forEach(office => appendInput(Number(office?.id || 0) || null));
        ...
        function togglePendingColumns() {
            ...
            if (!pendingVisible) {
                ...
                try {
                    ...
                    console.error("Failed to build pending inputs", error);
                }
                ensurePendingBodyCells();
                refreshMonthButtonState();
                refreshPendingInputs();
                refreshSummaryCards();
            } else {
        ...
        function addInputCells(sectionType) {
            ...
            if (isCarTotal) {
                ...
                const sectionTotal = getCurrentMonthSectionTotal(row, sourceSection, currentCol);
        ...
        function getProgramIdForDataRow(row, rowId = 0) {
            return Number(row?.dataset?.programId || rowId || 0);
        }

        function getSectionPayloadForRow(row, sectionType) {
            const indicatorId = Number(row.dataset.indicatorId || 0);
            const rowId = Number(row.dataset.rowId || 0);
            const programId = getProgramIdForDataRow(row, rowId);
        ...
        function collectPendingSectionEntries(sectionType) {
            return Array.from(document.querySelectorAll('tbody tr[data-row-id]'))
                .flatMap(row => {
                    const indicatorId = Number(row.dataset.indicatorId || 0);
                    const rowId = Number(row.dataset.rowId || 0);
                    ...
                    const currentCol = getCurrentMonthPeriodIndex();
                    const currentPeriodKey = PERIOD_KEYS[currentCol] || null;
                    if (!currentPeriodKey) return [];

                    const pendingInputs = Array.from(row.querySelectorAll(`.month-box[data-section="pending"][data-source-section="${sectionType}"]`))
                        .filter(input => input.dataset.carTotal !== '1'
                            && input.dataset.groupTotal !== '1'
                    ...
                    const sourceByIndicator = sectionType === 'target'
                        ? existingTargetsByIndicator
                        : existingAccompByIndicator;
                    const existingByOffice = ((sourceByIndicator[String(rowId)] || {})[String(indicatorId)]) || {};
                    const remarksByOffice = sectionType === 'accomp' ? getRemarksByOfficeForRow(row) : {};
                    const carInput = row.querySelector(`.month-box[data-section="pending"][data-source-section="${sectionType}"][data-col="${currentCol}"][data-car-total="1"]`);
                    const carValue = parsePeriodInputValue(carInput?.value ?? 0);

                    const officeIds = Array.from(new Set(pendingInputs
                        .map(input => String(input.dataset.officeId || '').trim())
                        .filter(Boolean)));
                    ...
                    return officeIds.map(officeId => {
                        ...
                        const matchingInput = pendingInputs.find(input => String(input.dataset.officeId || '') === String(officeId));
                        if (matchingInput) {
                            const value = parsePeriodInputValue(matchingInput.value);
                            entry[currentPeriodKey] = Number.isFinite(value) ? value : 0;
                        }

                        entry.car_totals = { ...(existingOfficeData.car_totals || {}) };
                        if (Number.isFinite(carValue)) {
                            entry.car_totals[currentPeriodKey] = carValue;
                        }
                        entry.group_totals = { ...(existingOfficeData.group_totals || {}) };
        ...
        function collectChangedAccomplishmentEntries() {
            ...
            [...collectAccomplishmentEntries(), ...collectPendingSectionEntries('accomp')]
                .filter(entry => hasEntryChanged('accomp', entry))
                .forEach(entry => {
                    const key = `${entry.row_id}|${entry.indicator_id}|${entry.office_id}`;
