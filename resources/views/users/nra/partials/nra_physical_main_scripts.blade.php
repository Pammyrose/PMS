    ...
    <script>
        ...
        const targetNrareUrl = @json(route('users.nra_physical.targets.store'));
        const accompNrareUrl = @json(route('users.nra_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.nra_physical.rows.destroy'));
        ...
        function buildMonthlyMapFromNrared(sourceData) {
        ...
        function refreshSummaryCards() {
            const targetMap = buildMonthlyMapFromNrared(existingTargetsByIndicator);
            const accompMap = buildMonthlyMapFromNrared(existingAccompByIndicator);
        ...
        function updateSummaryInputsForRow(row) {
            ...
            summaryInputs.forEach(input => {
                ...
                if (!Number.isFinite(value)) {
                    value = getNraredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey);
                }
        ...
        function getNraredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey) {
        ...
        function getNraredEntryByOffice(sourceByIndicator, programId, indicatorId, officeId) {
        ...
        function hasEntryChanged(sectionType, entry) {
            ...
            const sourceByIndicator = sectionType === 'target'
                ? existingTargetsByIndicator
                : existingAccompByIndicator;
            const storedEntry = getNraredEntryByOffice(sourceByIndicator, rowId, indicatorId, officeId);
        ...
        async function saveSectionEntries(sectionType, options = {}) {
            ...
            const url = isTarget ? targetNrareUrl : accompNrareUrl;
