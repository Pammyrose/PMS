    ...
    <script>
        ...
        const targetEngpreUrl = @json(route('users.engp_physical.targets.store'));
        const accompEngpreUrl = @json(route('users.engp_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.engp_physical.rows.destroy'));
        ...
        function buildMonthlyMapFromEngpred(sourceData) {
        ...
        function refreshSummaryCards() {
            const targetMap = buildMonthlyMapFromEngpred(existingTargetsByIndicator);
            const accompMap = buildMonthlyMapFromEngpred(existingAccompByIndicator);
        ...
        function updateSummaryInputsForRow(row) {
            ...
            summaryInputs.forEach(input => {
                ...
                if (!Number.isFinite(value)) {
                    value = getEngpredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey);
                }
        ...
        function getEngpredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey) {
        ...
        function getEngpredEntryByOffice(sourceByIndicator, programId, indicatorId, officeId) {
        ...
        function hasEntryChanged(sectionType, entry) {
            ...
            const sourceByIndicator = sectionType === 'target'
                ? existingTargetsByIndicator
                : existingAccompByIndicator;
            const storedEntry = getEngpredEntryByOffice(sourceByIndicator, rowId, indicatorId, officeId);
        ...
        async function saveSectionEntries(sectionType, options = {}) {
            ...
            const url = isTarget ? targetEngpreUrl : accompEngpreUrl;
