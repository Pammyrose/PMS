    ...
    <script>
        ...
        const targetEnfreUrl = @json(route('users.enf_physical.targets.store'));
        const accompEnfreUrl = @json(route('users.enf_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.enf_physical.rows.destroy'));
        ...
        function buildMonthlyMapFromEnfred(sourceData) {
        ...
        function refreshSummaryCards() {
            const targetMap = buildMonthlyMapFromEnfred(existingTargetsByIndicator);
            const accompMap = buildMonthlyMapFromEnfred(existingAccompByIndicator);
        ...
        function updateSummaryInputsForRow(row) {
            ...
            summaryInputs.forEach(input => {
                ...
                if (!Number.isFinite(value)) {
                    value = getEnfredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey);
                }
        ...
        function getEnfredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey) {
        ...
        function getEnfredEntryByOffice(sourceByIndicator, programId, indicatorId, officeId) {
        ...
        function hasEntryChanged(sectionType, entry) {
            ...
            const sourceByIndicator = sectionType === 'target'
                ? existingTargetsByIndicator
                : existingAccompByIndicator;
            const storedEntry = getEnfredEntryByOffice(sourceByIndicator, rowId, indicatorId, officeId);
        ...
        async function saveSectionEntries(sectionType, options = {}) {
            ...
            const url = isTarget ? targetEnfreUrl : accompEnfreUrl;
