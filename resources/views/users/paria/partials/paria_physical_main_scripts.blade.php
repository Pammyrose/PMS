    ...
    <script>
        ...
        const targetPariareUrl = @json(route('users.paria_physical.targets.store'));
        const accompPariareUrl = @json(route('users.paria_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.paria_physical.rows.destroy'));
        ...
        function buildMonthlyMapFromPariared(sourceData) {
        ...
        function refreshSummaryCards() {
            const targetMap = buildMonthlyMapFromPariared(existingTargetsByIndicator);
            const accompMap = buildMonthlyMapFromPariared(existingAccompByIndicator);
        ...
        function updateSummaryInputsForRow(row) {
            ...
            summaryInputs.forEach(input => {
                ...
                if (!Number.isFinite(value)) {
                    value = getPariaredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey);
                }
        ...
        function getPariaredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey) {
        ...
        function getPendingComparisonTotal(row, sourceSection, colIndex) {
            return getCurrentMonthSectionTotal(row, sourceSection, colIndex);
        }
                ...
                ['target', 'accomp'].forEach(sourceSection => {
                    ...
                    pendingInputs.forEach(input => {
                        ...
                        if (input.dataset.carTotal === '1') {
                            input.value = getCurrentMonthSectionTotal(row, sourceSection, colIndex);
                            return;
                        }
            ...
            if (sectionType === 'summary') {
                ...
                const buildPendingValueInput = ({ sourceSection, officeId, isCarTotal = false, isGroupTotal = false, groupOfficeIds = [] }) => {
                    ...
                    if (isCarTotal) {
                        ...
                        input.value = getCurrentMonthSectionTotal(row, sourceSection, currentCol);
                        return input;
                    }
        ...
        function getPariaredEntryByOffice(sourceByIndicator, programId, indicatorId, officeId) {
        ...
        function hasEntryChanged(sectionType, entry) {
            ...
            const sourceByIndicator = sectionType === 'target'
                ? existingTargetsByIndicator
                : existingAccompByIndicator;
            const storedEntry = getPariaredEntryByOffice(sourceByIndicator, rowId, indicatorId, officeId);
        ...
        async function saveSectionEntries(sectionType, options = {}) {
            ...
            const url = isTarget ? targetPariareUrl : accompPariareUrl;
