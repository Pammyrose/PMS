    ...
    <script>
        ...
        const targetCobbreUrl = @json(route('users.cobb_physical.targets.store'));
        const accompCobbreUrl = @json(route('users.cobb_physical.accomplishments.store'));
        const deletePhysicalRowUrl = @json(route('admin.cobb_physical.rows.destroy'));
        ...
        function buildMonthlyMapFromCobbred(sourceData) {
        ...
        function refreshSummaryCards() {
            const targetMap = buildMonthlyMapFromCobbred(existingTargetsByIndicator);
            const accompMap = buildMonthlyMapFromCobbred(existingAccompByIndicator);
        ...
        function updateSummaryInputsForRow(row) {
            ...
            summaryInputs.forEach(input => {
                ...
                if (!Number.isFinite(value)) {
                    value = getCobbredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey);
                }
        ...
        function getCobbredValueForSummary(sectionType, programId, indicatorId, officeId, periodKey) {
        ...
        function getCobbredEntryByOffice(sourceByIndicator, programId, indicatorId, officeId) {
        ...
        function hasEntryChanged(sectionType, entry) {
            ...
            const sourceByIndicator = sectionType === 'target'
                ? existingTargetsByIndicator
                : existingAccompByIndicator;
            const storedEntry = getCobbredEntryByOffice(sourceByIndicator, rowId, indicatorId, officeId);
        ...
        async function saveSectionEntries(sectionType, options = {}) {
            ...
            const url = isTarget ? targetCobbreUrl : accompCobbreUrl;
