    ...
    <script>
        ...
        function getStoredPendingTotal(row, colIndex) {
            ...
            const value = targetTotal > accompTotal ? targetTotal - accompTotal : 0;
        ...
        function rowHasCurrentMonthPending(row) {
            ...
            const targetValue = getPendingComparisonTotal(row, 'target', colIndex);
            const accompValue = getPendingComparisonTotal(row, 'accomp', colIndex);


            if (Math.abs(targetValue - accompValue) < 0.000001) {
                return false;
            }

            return targetValue > accompValue;
        }
                ...
                PERIODS.forEach((period, idx) => {
                    ...
                    officeEntries.forEach((office, officeIndex) => {
                        ...
                        if (sectionType === 'summary' || sectionType === 'pending' || (period.type !== "month" && !isExcelImportedOfficeData)) {
                            ...
                            input.readOnly = sectionType === 'target' || period.type === 'annual';
                        }
