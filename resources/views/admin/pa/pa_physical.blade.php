...
>
                                        ...
                                        <td class="px-6 py-4" colspan="4">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span class="flex items-center">
                                                    ...
                                                    @endif
                                                    <form method="POST" action="{{ route('admin.pa.pap.destroy', $program) }}"
    ...
    <script>
        ...
        const targetStoreUrl = @json(route('admin.pa.targets.store'));
        const accompStoreUrl = @json(route('admin.pa.accomplishments.store'));
    ...
    <div class="modal fade" id="addIndicatorModal" tabindex="-1" aria-labelledby="addIndicatorModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content border-0 shadow-lg">
                ...
                <form id="addIndicatorForm" method="POST" action="{{ route('admin.pa.indicators.store') }}"
                    data-update-route-template="{{ route('admin.pa.indicators.update', ':id') }}"
                    data-delete-route-template="{{ route('admin.pa.indicators.destroy', ':id') }}">
                    @csrf
        ...
        document.getElementById('addIndicatorForm')?.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (!programId) {
                    ...
                    const papResponse = await fetch(@json(route('admin.pa.pap.store')), {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': token,
