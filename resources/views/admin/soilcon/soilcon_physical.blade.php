...
<body class="bg-light">
                                            ...
                                            <td class="px-6 py-4" colspan="3">
                                                <div class="flex items-center justify-between">
                                                    ...
                                                    <span class="flex items-center">
                                                        ...
                                                        @endif
                                                        <form method="POST"
                                                            action="{{ route('admin.soilcon_physical.pap.destroy', ['program' => $program->id]) }}"
    ...
    <script>
        ...
        const targetStoreUrl = @json(route('admin.soilcon_physical.targets.store'));
        const accompStoreUrl = @json(route('admin.soilcon_physical.accomplishments.store'));
    ...
    <div class="modal fade" id="addIndicatorModal" tabindex="-1" aria-labelledby="addIndicatorModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content border-0 shadow-lg">
                ...
                <form id="addIndicatorForm" method="POST" action="{{ route('admin.soilcon_physical.indicators.store') }}"
                    data-update-route-template="{{ route('admin.soilcon_physical.indicators.update', ':id') }}"
                    data-delete-route-template="{{ route('admin.soilcon_physical.indicators.destroy', ':id') }}">
                    @csrf
        ...
        document.getElementById('addIndicatorForm')?.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (!programId) {
                    ...
                    const papResponse = await fetch(@json(route('admin.soilcon_physical.pap.store')), {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': token,
