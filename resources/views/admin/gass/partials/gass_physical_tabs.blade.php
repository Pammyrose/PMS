...
<div class="d-flex justify-content-end align-items-center mt-3 mb-1">
    <form method="GET" action="{{ url()->current() }}" class="d-flex align-items-center gap-2" id="yearFilterForm">
        ...
        <select id="year_filter" name="year"
            ...
            onchange="updateYearHeader(this); this.form.submit()">
            @php
...
<script>
    function updateYearHeader(selectElement) {
        const selectedYear = selectElement.value;
        const headerYearSpan = document.getElementById('headerYear');
        if (headerYearSpan) {
            headerYearSpan.textContent = selectedYear;
        }
    }
</script>
