    ...
    <div class="modal fade" id="addIndicatorModal" tabindex="-1" aria-labelledby="addIndicatorModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content border-0 shadow-lg">
                ...
                <form id="addIndicatorForm" method="POST" action="{{ route('admin.continuing_physical.indicators.store') }}"
                    data-update-route-template="{{ route('admin.continuing_physical.indicators.update', ':id') }}"
                    data-delete-route-template="{{ route('admin.continuing_physical.indicators.destroy', ':id') }}">
                    @csrf
        ...
        document.getElementById('addIndicatorForm')?.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (!programId) {
                    ...
                    const papResponse = await fetch(@json(route('admin.continuing_physical.pap.store')), {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': token,
