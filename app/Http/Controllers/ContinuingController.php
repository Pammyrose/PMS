...
{
    public function index(Request $request, Continuing_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Continuing_Pap::query();
        ...
        $indicators = Continuing_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Continuing_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Continuing_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papTitles = Continuing_Pap::query()
            ->select('title')
            ->whereNotNull('title')
            ->where('title', '!=', '')
            ->distinct()
        ...
        $papProjects = Continuing_Pap::query()
            ->select('project')
        ...
        $papActivities = Continuing_Pap::query()
            ->select('activities')
        ...
        $papSubactivities = Continuing_Pap::query()
        ...
        $papPrefillData = Continuing_Pap::query()
            ->with('indicators')
        ...
        return view('admin.continuing.continuing_physical', compact(
            'entries',
            'programs',
            'existing',
            'indicators',
    ...
    public function storePap(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $pap = Continuing_Pap::create($validated);
            DB::commit();
    ...
    public function destroyPap(Continuing_Pap $program)
    {
        DB::beginTransaction();
        try {
            Continuing_Target::where('program_id', $program->id)->delete();
            Continuing_Accomplishment::where('program_id', $program->id)->delete();
            Continuing_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:continuing_pap,id',
        ...
        $indicator = Continuing_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:continuing_pap,id',
            'entries.*.indicator_id' => 'required|exists:continuing_indicators,id',
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Continuing_Target::firstOrNew([
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Continuing_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
