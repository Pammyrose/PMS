...
class EnfController extends Controller
{
    ...
    {
        $allPrograms = $this->getEnfPrograms();
    ...
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.enf_physical.physical', $firstProgramId)
            : route('admin.enf_physical.physical');
    ...
    private function hierarchySortValue($value, bool $singleIAsRoman = true): string
    {
        ...
        if (preg_match('/^menu\s+(\d+)\s*[:.]?/i', $raw, $matches)) {
            return '0|' . str_pad($matches[1], 6, '0', STR_PAD_LEFT) . '.000000.000000.000000.000000|' . $normalized;
        }

        if (preg_match('/^([a-z])\.?\s*(\d+(?:\.\d+)*)(?:\s*\.?\s*([a-z])(?=[.)]))?[.)]\s*/i', $raw, $matches)) {
            $segments = [ord(strtoupper($matches[1])) - 64];
            $segments = array_merge($segments, array_map('intval', explode('.', $matches[2])));
            if (($matches[3] ?? '') !== '') {
                $segments[] = ord(strtoupper($matches[3])) - 64;
            }
        ...
        if (preg_match('/^(\d+)([a-z])[.)]\s*/i', $raw, $matches)) {
            $numericKey = str_pad($matches[1], 6, '0', STR_PAD_LEFT)
                . '.' . str_pad((string) (ord(strtoupper($matches[2])) - 64), 6, '0', STR_PAD_LEFT)
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findEnfProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Enf_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Enf_Accomplishment::query()->get());
    ...
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getEnfTypeId())
                ->where('record_type_id', $this->getEnfRecordTypeIds()['PROGRAM']);
        });
    ...
    {
        $program = $this->findEnfProgram((int) $program);
...
public function update(Request $request, Enf_Indicator $indicator)
{
    ...
    if ($shouldCreateSnapshot) {
        $newIndicator = new Enf_Indicator();
...
public function destroyIndicator(Request $request, Enf_Indicator $indicator)
{
    $indicator->delete();
...
public function storeTargets(Request $request)
{
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Enf_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Enf_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Enf_Accomplishment();
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Enf_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds, ?int $year = null): Collection
{
    ...
    $indicatorsById = Enf_Indicator::query()
...
private function findEnfProgram(int $programId): ?object
{
    return $this->getEnfPrograms($programId)->first();
}
...
private function getEnfRecordTypeIds(): array
{
