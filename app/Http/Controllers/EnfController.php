...
{
    public function index(Request $request, Enf_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Enf_Pap::query();
        ...
        $indicators = Enf_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Enf_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Enf_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Enf_Pap::query()
    ...
    public function destroyPap(Enf_Pap $program)
    {
        DB::beginTransaction();
        try {
            Enf_Target::where('program_id', $program->id)->delete();
            Enf_Accomplishment::where('program_id', $program->id)->delete();
            Enf_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:enf_pap,id',
        ...
        $indicator = Enf_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:enf_pap,id',
            'entries.*.indicator_id' => 'required|exists:enf_indicators,id',
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Enf_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
