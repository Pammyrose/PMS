...
{
    public function index(Request $request, Paria_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Paria_Pap::query();
        ...
        $indicators = Paria_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Paria_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Paria_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Paria_Pap::query()
        ...
        $papPrefillData = Paria_Pap::query()
            ->with('indicators')
    ...
    public function storePap(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $pap = Paria_Pap::create($validated);
            DB::commit();
    ...
    public function destroyPap(Paria_Pap $program)
    {
        DB::beginTransaction();
        try {
            Paria_Target::where('program_id', $program->id)->delete();
            Paria_Accomplishment::where('program_id', $program->id)->delete();
            Paria_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:paria_pap,id',
        ...
        $indicator = Paria_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:paria_pap,id',
            'entries.*.indicator_id' => 'required|exists:paria_indicators,id',
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Paria_Target::firstOrNew([
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Paria_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
