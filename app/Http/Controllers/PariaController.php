...
use App\Models\Paria_Target;
use App\Models\Paria_Accomplishment;
...
class PariaController extends Controller
{
    /**
    ...
    public function index(Request $request, $program = null)
    {
        ...
        return view($this->roleView('paria.paria_physical'), compact(
            'entries',
            'programs',
            'programsRaw',
            'existing',
    ...
    {
        $allPrograms = $this->getPariaPrograms();
        ...
        $programs = $grouped;
        return view('admin.paria.sto', compact('programs', 'offices'));
    }

    /**
     * Pariare or update physical accomplishment entries
     */
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.paria.physical', $firstProgramId)
            : route('admin.paria.physical');
    ...
    {
        $indicators = Paria_Indicator::latest()->get();

        return view('admin.paria.indicators', compact('indicators'));
    }
    ...
    {
        return view('admin.paria.indicator_create');
    }
    ...
    {
        $typeId = $this->getPariaTypeId();
        $recordTypeIds = $this->getPariaRecordTypeIds();
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findPariaProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Paria_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Paria_Accomplishment::query()->get());
    ...
    /**
     * Pariare new indicator
     */
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getPariaTypeId())
                ->where('record_type_id', $this->getPariaRecordTypeIds()['PROGRAM']);
        });
    ...
    {
        $program = $this->findPariaProgram((int) $program);

        if (!$program) {
            abort(404);
        }

        return view('admin.paria.program_edit', compact('program'));
    }

public function update(Request $request, Paria_Indicator $indicator)
{
    $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
        $query->where('types_id', $this->getPariaTypeId())
            ->where('record_type_id', $this->getPariaRecordTypeIds()['PROGRAM']);
...
public function destroyIndicator(Request $request, Paria_Indicator $indicator)
{
    $indicator->delete();
...
public function destroyPhysicalRow(Request $request)
{
    $validated = $request->validate([
        'row_id' => [
            'required',
            'integer',
            Rule::exists('ppa', 'id')->where(fn ($query) => $query->where('types_id', $this->getPariaTypeId())),
        ],
    ...
    try {
        $ppaRow = DB::table('ppa')
            ->where('id', $rowId)
            ->where('types_id', $this->getPariaTypeId())
            ->first();
        ...
        $deletedTargets = $this->deleteSectionRowsForPhysicalRows(Paria_Target::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
        $deletedAccomplishments = $this->deleteSectionRowsForPhysicalRows(Paria_Accomplishment::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);

        $indicatorNames = empty($indicatorIds)
            ? []
            : Paria_Indicator::query()
...
public function storeTargets(Request $request)
{
    ...
    $existingRows = Paria_Target::query()
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Paria_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Paria_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Paria_Accomplishment();
        ...
        $rawRemarks = array_key_exists('remarks', $entry)
            ? ($entry['remarks'] ?? null)
            : $this->decodeRemarksFromPariarage($record->remarks ?? null);
        $record->remarks = $this->encodeRemarksForPariarage($rawRemarks);
        $record->values = json_encode([
            'user_id' => $userId,
            'program_id' => $programId,
            'row_id' => $rowId,
...
private function formatSectionRecordForJs($row, array $meta = []): array
{
    return [
        ...
        'remarks' => $this->decodeRemarksFromPariarage($row->remarks ?? null),
    ];
}

private function encodeRemarksForPariarage($remarks): ?string
{
    if ($remarks === null) {
        return null;
    }
...
private function decodeRemarksFromPariarage($raw): string
{
    if ($raw === null) {
        return '';
    }
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Paria_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds): Collection
{
    ...
    $indicatorsById = Paria_Indicator::query()
...
private function getPariaPrograms(?int $programId = null, string $search = '', ?int $year = null): Collection
{
    $recordTypeIds = $this->getPariaRecordTypeIds();
    $typeId = $this->getPariaTypeId();
...
private function findPariaProgram(int $programId): ?object
{
    return $this->getPariaPrograms($programId)->first();
}
...
private function getPariaTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('PARIA type is not configured.');
    }

    return (int) $typeId;
}

private function getPariaRecordTypeIds(): array
{
