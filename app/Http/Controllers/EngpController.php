...
class EngpController extends Controller
{
    ...
    public function index(Request $request, $program = null)
    {
        ...
        return view($this->roleView('engp.engp_physical'), compact(
            'entries',
            'programs',
            'programsRaw',
            'existing',
    ...
    {
        $allPrograms = $this->getEngpPrograms();
        ...
        $programs = $grouped;
        return view('admin.engp.engp', compact('programs', 'offices'));
    }

    /**
     * Engpre or update physical accomplishment entries
     */
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.engp.physical', $firstProgramId)
            : route('admin.engp.physical');
    ...
    {
        $indicators = Engp_Indicator::latest()->get();

        return view('admin.engp.indicators', compact('indicators'));
    }
    ...
    {
        return view('admin.engp.indicator_create');
    }
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findEngpProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Engp_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Engp_Accomplishment::query()->get());
    ...
    /**
     * Engpre new indicator
     */
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getEngpTypeId())
                ->where('record_type_id', $this->getEngpRecordTypeIds()['PROGRAM']);
        });
    ...
    {
        $program = $this->findEngpProgram((int) $program);

        if (!$program) {
            abort(404);
        }

        return view('admin.engp.program_edit', compact('program'));
    }
...
public function destroyPhysicalRow(Request $request)
{
    $validated = $request->validate([
        'row_id' => [
            'required',
            'integer',
            Rule::exists('ppa', 'id')->where(fn ($query) => $query->where('types_id', $this->getEngpTypeId())),
        ],
    ...
    try {
        $ppaRow = DB::table('ppa')
            ->where('id', $rowId)
            ->where('types_id', $this->getEngpTypeId())
            ->first();
        ...
        $deletedTargets = $this->deleteSectionRowsForPhysicalRows(Engp_Target::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
        $deletedAccomplishments = $this->deleteSectionRowsForPhysicalRows(Engp_Accomplishment::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);

        $indicatorNames = empty($indicatorIds)
            ? []
            : Engp_Indicator::query()
...
public function storeTargets(Request $request)
{
    ...
    $existingRows = Engp_Target::query()
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Engp_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Engp_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Engp_Accomplishment();
        ...
        $rawRemarks = array_key_exists('remarks', $entry)
            ? ($entry['remarks'] ?? null)
            : $this->decodeRemarksFromEngprage($record->remarks ?? null);
        $record->remarks = $this->encodeRemarksForEngprage($rawRemarks);
        $record->values = json_encode([
            'user_id' => $userId,
            'program_id' => $programId,
            'row_id' => $rowId,
...
private function formatSectionRecordForJs($row, array $meta = []): array
{
    return [
        ...
        'remarks' => $this->decodeRemarksFromEngprage($row->remarks ?? null),
    ];
}

private function encodeRemarksForEngprage($remarks): ?string
{
    if ($remarks === null) {
        return null;
    }
...
private function decodeRemarksFromEngprage($raw): string
{
    if ($raw === null) {
        return '';
    }
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Engp_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds): Collection
{
    ...
    $indicatorsById = Engp_Indicator::query()
...
private function getEngpPrograms(?int $programId = null, string $search = '', ?int $year = null): Collection
{
    $recordTypeIds = $this->getEngpRecordTypeIds();
    $typeId = $this->getEngpTypeId();
...
private function findEngpProgram(int $programId): ?object
{
    return $this->getEngpPrograms($programId)->first();
}
...
private function getEngpTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('ENGP type is not configured.');
    }

    return (int) $typeId;
}

private function getEngpRecordTypeIds(): array
{
