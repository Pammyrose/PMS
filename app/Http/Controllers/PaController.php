...
{
    public function index(Request $request, Pa_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Pa_Pap::query();
        ...
        $indicators = Pa_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Pa_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Pa_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
    ...
    public function destroyPap(Pa_Pap $program)
    {
        DB::beginTransaction();
        try {
            Pa_Target::where('program_id', $program->id)->delete();
            Pa_Accomplishment::where('program_id', $program->id)->delete();
            Pa_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:pa_pap,id',
        ...
        $indicator = Pa_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:pa_pap,id',
            'entries.*.indicator_id' => 'required|exists:pa_indicators,id',
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Pa_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
