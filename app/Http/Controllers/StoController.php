...
{
    public function index(Request $request, Sto_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Sto_Pap::query();
        ...
        $indicators = Sto_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Sto_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Sto_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Sto_Pap::query()
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:sto_pap,id',
        ...
        $indicator = Sto_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:sto_pap,id',
            'entries.*.indicator_id' => 'required|exists:sto_indicators,id',
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Sto_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
