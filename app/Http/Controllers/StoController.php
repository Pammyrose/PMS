...
use App\Models\Sto_Accomplishment;
...
class StoController extends Controller
{
    ...
    public function index(Request $request, $program = null)
    {
        ...
        $programsRaw = $this->getStoPrograms($programId, $search, (int) $year)
        ...
        $targets = Sto_Target::where('years', $year)
            ->when($this->shouldScopeToUserOffice(), fn ($query) => $query->where('office_ids', $office_id))
            ->get();
        $accomplishments = Sto_Accomplishment::where('years', $year)
            ->when($this->shouldScopeToUserOffice(), fn ($query) => $query->where('office_ids', $office_id))
            ->get();
        ...
        return view($this->roleView('sto.sto_physical'), compact(
            'entries',
            'programs',
            'programsRaw',
            'existing',
    ...
    {
        $allPrograms = $this->getStoPrograms();
        ...
        $programs = $grouped;
        return view('admin.sto.sto', compact('programs', 'offices'));
    }
    ...
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.sto_physical.physical', $firstProgramId)
            : route('admin.sto_physical.physical');
    ...
    {
        $indicators = Sto_Indicator::latest()->get();

        return view('admin.sto.indicators', compact('indicators'));
    }
    ...
    {
        return view('admin.sto.indicator_create');
    }
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findStoProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Sto_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Sto_Accomplishment::query()->get());
    ...
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getStoTypeId())
                ->where('record_type_id', $this->getStoRecordTypeIds()['PROGRAM']);
        });
    ...
    {
        $program = $this->findStoProgram((int) $program);

        if (!$program) {
            abort(404);
        }

        return view('admin.sto.program_edit', compact('program'));
    }

public function update(Request $request, Sto_Indicator $indicator)
{
    $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
        $query->where('types_id', $this->getStoTypeId())
            ->where('record_type_id', $this->getStoRecordTypeIds()['PROGRAM']);
...
public function destroyIndicator(Request $request, Sto_Indicator $indicator)
{
    $indicator->delete();
...
public function destroyPhysicalRow(Request $request)
{
    $validated = $request->validate([
        'row_id' => [
            'required',
            'integer',
            Rule::exists('ppa', 'id')->where(fn ($query) => $query->where('types_id', $this->getStoTypeId())),
        ],
    ...
    try {
        ...
        $deletedTargets = $this->deleteSectionRowsForPhysicalRows(Sto_Target::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
        $deletedAccomplishments = $this->deleteSectionRowsForPhysicalRows(Sto_Accomplishment::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
...
public function storeTargets(Request $request)
{
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Sto_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Sto_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Sto_Accomplishment();
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Sto_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds, ?int $year = null): Collection
{
    ...
    $indicatorsById = Sto_Indicator::query()
...
private function getStoPrograms(?int $programId = null, string $search = '', ?int $year = null): Collection
{
    $recordTypeIds = $this->getStoRecordTypeIds();
    $typeId = $this->getStoTypeId();
...
private function findStoProgram(int $programId): ?object
{
    return $this->getStoPrograms($programId)->first();
}
...
private function getStoTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('STO type is not configured.');
    }

    return (int) $typeId;
}

private function getStoRecordTypeIds(): array
{
