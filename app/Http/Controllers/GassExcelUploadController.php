...
class GassExcelUploadController extends GassController
{
    public function importExcel(Request $request)
    {
        ...
        try {
            $result = $this->importGassPhysicalRowsFromExcel($filePath, $year);
            DB::commit();

            return redirect()
                ->back()
                ->with('success', 'GASS Excel import complete.');
        } catch (\Throwable $exception) {
            DB::rollBack();

            return redirect()
                ->back()
                ->with('error', 'GASS Excel import failed: ' . $exception->getMessage());
        }
    }
    ...
    public function previewExcelImport(Request $request)
    {
        ...
        try {
            return response()->json([
                'success' => true,
                'preview' => $this->previewGassPhysicalRowsFromExcel($filePath, $year),
            ]);
        } catch (\Throwable $exception) {
            return response()->json([
                'success' => false,
                'message' => 'GASS Excel preview failed: ' . $exception->getMessage(),
            ], 422);
        }
    }

    private function importGassPhysicalRowsFromExcel(string $filePath, int $year): array
    {
        ...
        $startPlaceholderBlock = function (int $sourceRow, array $targetValues = []) use (&$currentBlock, &$currentHeaders, &$currentProgram, &$currentProgramMeta) {
            ...
            $currentBlock = [
                ...
                'title' => $currentProgram ?: 'GENERAL ADMINISTRATION AND SUPPORT SERVICES (GASS)',
        ...
        foreach ($reader->rows($filePath, 'GASS', true) as $rowNumber => $row) {
            if ($rowNumber < 10) {
                continue;
            }
            ...
            if ($this->isGassProgramHeader($papText, $row)) {
                $flushBlock();
                if ($this->isDuplicateGassSummaryProgramHeader($papText)) {
    ...
    private function previewGassPhysicalRowsFromExcel(string $filePath, int $year): array
    {
        $reader = new SimpleXlsxReader();
        $officeMap = $this->getOfficeImportMap();
        $currentProgram = null;
    ...
    private function papDataFromExcelBlock(array $block, int $year): array
    {
        ...
        if (!empty($block['standalone_title']) && !empty($headers)) {
            ...
            return [
                'title' => $this->excelPpaName($standaloneTitle ?: ($block['title'] ?: 'GENERAL ADMINISTRATION AND SUPPORT SERVICES (GASS)')),
        ...
        return [
            'title' => $this->excelPpaName($block['title'] ?: 'GENERAL ADMINISTRATION AND SUPPORT SERVICES (GASS)'),
    ...
    private function firstOrCreateImportedIndicator(string $indicatorName, ?int $indicatorTypeId = null): Gass_Indicator
    {
        $indicator = Gass_Indicator::query()
    ...
    private function isGassProgramHeader(string $text, array $row): bool
    {
    ...
    private function isDuplicateGassSummaryProgramHeader(string $text): bool
    {
        $normalized = preg_replace('/[^A-Z0-9]+/', '', strtoupper(trim($text)));
