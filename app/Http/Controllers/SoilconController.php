...
{
    public function index(Request $request, Soilcon_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Soilcon_Pap::query();
        ...
        $indicators = Soilcon_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Soilcon_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Soilcon_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papActivities = Soilcon_Pap::query()
            ->select('activities')
        ...
        $papSubactivities = Soilcon_Pap::query()
        ...
        $papPrefillData = Soilcon_Pap::query()
            ->with('indicators')
    ...
    public function storePap(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $pap = Soilcon_Pap::create($validated);
            DB::commit();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:soilcon_pap,id',
        ...
        $indicator = Soilcon_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:soilcon_pap,id',
            'entries.*.indicator_id' => 'required|exists:soilcon_indicators,id',
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Soilcon_Target::firstOrNew([
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Soilcon_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
