...
use App\Models\Soilcon_Target;
use App\Models\Soilcon_Accomplishment;
...
class SoilconController extends Controller
{
    ...
    public function index(Request $request, $program = null)
    {
        ...
        $programsRaw = $this->getSoilconPrograms($programId, $search, (int) $year)
        ...
        $targets = Soilcon_Target::where('years', $year)
        ...
        $accomplishments = Soilcon_Accomplishment::where('years', $year)
            ->when($this->shouldScopeToUserOffice(), fn ($query) => $query->where('office_ids', $office_id))
            ->get()
            ->reduce(function (array $carry, $row) {
                $meta = $this->parseSectionValues($row->values ?? null);
        ...
        return view($this->roleView('soilcon.soilcon_physical'), compact(
            'entries',
            'programs',
            'programsRaw',
            'existing',
    ...
    {
        $allPrograms = $this->getSoilconPrograms();
        ...
        $programs = $grouped;
        return view('admin.soilcon.sto', compact('programs', 'offices'));
    }

    /**
     * Soilconre or update physical accomplishment entries
     */
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.soilcon.physical', $firstProgramId)
            : route('admin.soilcon.physical');
    ...
    {
        $indicators = Soilcon_Indicator::latest()->get();

        return view('admin.soilcon.indicators', compact('indicators'));
    }
    ...
    {
        return view('admin.soilcon.indicator_create');
    }
    ...
    {
        $typeId = $this->getSoilconTypeId();
        $recordTypeIds = $this->getSoilconRecordTypeIds();
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findSoilconProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Soilcon_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Soilcon_Accomplishment::query()->get());
    ...
    /**
     * Soilconre new indicator
     */
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getSoilconTypeId())
                ->where('record_type_id', $this->getSoilconRecordTypeIds()['PROGRAM']);
        });
        ...
        $indicator = new Soilcon_Indicator();
    ...
    {
        $program = $this->findSoilconProgram((int) $program);

        if (!$program) {
            abort(404);
        }

        return view('admin.soilcon.program_edit', compact('program'));
    }

public function update(Request $request, Soilcon_Indicator $indicator)
{
    $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
        $query->where('types_id', $this->getSoilconTypeId())
            ->where('record_type_id', $this->getSoilconRecordTypeIds()['PROGRAM']);
...
public function destroyIndicator(Request $request, Soilcon_Indicator $indicator)
{
    $indicator->delete();
...
public function destroyPhysicalRow(Request $request)
{
    $validated = $request->validate([
        'row_id' => [
            'required',
            'integer',
            Rule::exists('ppa', 'id')->where(fn ($query) => $query->where('types_id', $this->getSoilconTypeId())),
        ],
    ...
    try {
        $ppaRow = DB::table('ppa')
            ->where('id', $rowId)
            ->where('types_id', $this->getSoilconTypeId())
            ->first();
        ...
        $deletedTargets = $this->deleteSectionRowsForPhysicalRows(Soilcon_Target::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
        $deletedAccomplishments = $this->deleteSectionRowsForPhysicalRows(Soilcon_Accomplishment::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);

        $indicatorNames = empty($indicatorIds)
            ? []
            : Soilcon_Indicator::query()
...
public function storeTargets(Request $request)
{
    ...
    $existingRows = Soilcon_Target::query()
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Soilcon_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Soilcon_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Soilcon_Accomplishment();
        ...
        $rawRemarks = array_key_exists('remarks', $entry)
            ? ($entry['remarks'] ?? null)
            : $this->decodeRemarksFromSoilconrage($record->remarks ?? null);
        $record->remarks = $this->encodeRemarksForSoilconrage($rawRemarks);
        $record->values = json_encode([
            'user_id' => $userId,
            'program_id' => $programId,
            'row_id' => $rowId,
...
private function formatSectionRecordForJs($row, array $meta = []): array
{
    return [
        ...
        'remarks' => $this->decodeRemarksFromSoilconrage($row->remarks ?? null),
    ];
}

private function encodeRemarksForSoilconrage($remarks): ?string
{
    if ($remarks === null) {
        return null;
    }
...
private function decodeRemarksFromSoilconrage($raw): string
{
    if ($raw === null) {
        return '';
    }
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Soilcon_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds): Collection
{
    ...
    $indicatorsById = Soilcon_Indicator::query()
...
private function getSoilconPrograms(?int $programId = null, string $search = '', ?int $year = null): Collection
{
    $recordTypeIds = $this->getSoilconRecordTypeIds();
    $typeId = $this->getSoilconTypeId();
...
private function findSoilconProgram(int $programId): ?object
{
    return $this->getSoilconPrograms($programId)->first();
}
...
private function getSoilconTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('Soilcon type is not configured.');
    }

    return (int) $typeId;
}

private function getSoilconRecordTypeIds(): array
{
