...
{
    public function index(Request $request, Lands_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Lands_Pap::query();
        ...
        $indicators = Lands_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Lands_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Lands_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Lands_Pap::query()
        ...
        $papPrefillData = Lands_Pap::query()
            ->with('indicators')
    ...
    public function storePap(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $pap = Lands_Pap::create($validated);
            DB::commit();
    ...
    public function destroyPap(Lands_Pap $program)
    {
        DB::beginTransaction();
        try {
            Lands_Target::where('program_id', $program->id)->delete();
            Lands_Accomplishment::where('program_id', $program->id)->delete();
            Lands_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:lands_pap,id',
        ...
        $indicator = Lands_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:lands_pap,id',
            'entries.*.indicator_id' => 'required|exists:lands_indicators,id',
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Lands_Target::firstOrNew([
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Lands_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
