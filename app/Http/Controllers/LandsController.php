...
use App\Models\Lands_Target;
use App\Models\Lands_Accomplishment;
...
class LandsController extends Controller
{
    ...
    public function index(Request $request, $program = null)
    {
        ...
        $programsRaw = $this->getLandsPrograms($programId, $search, (int) $year)
        ...
        $targets = Lands_Target::where('years', $year)
        ...
        $accomplishments = Lands_Accomplishment::where('years', $year)
            ->when($this->shouldScopeToUserOffice(), fn ($query) => $query->where('office_ids', $office_id))
            ->get()
            ->reduce(function (array $carry, $row) {
                $meta = $this->parseSectionValues($row->values ?? null);
        ...
        return view($this->roleView('lands.lands_physical'), compact(
            'entries',
            'programs',
            'programsRaw',
            'existing',
    ...
    {
        $allPrograms = $this->getLandsPrograms();
        ...
        $programs = $grouped;
        return view('admin.lands.lands', compact('programs', 'offices'));
    }

    /**
     * Landsre or update physical accomplishment entries
     */
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.lands.physical', $firstProgramId)
            : route('admin.lands.physical');
    ...
    {
        $indicators = Lands_Indicator::latest()->get();

        return view('admin.lands.indicators', compact('indicators'));
    }
    ...
    {
        return view('admin.lands.indicator_create');
    }
    ...
    {
        $typeId = $this->getLandsTypeId();
        $recordTypeIds = $this->getLandsRecordTypeIds();
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findLandsProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Lands_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Lands_Accomplishment::query()->get());
    ...
    /**
     * Landsre new indicator
     */
    public function storeIndicator(Request $request)
    {
        $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
            $query->where('types_id', $this->getLandsTypeId())
                ->where('record_type_id', $this->getLandsRecordTypeIds()['PROGRAM']);
        });
    ...
    {
        $program = $this->findLandsProgram((int) $program);

        if (!$program) {
            abort(404);
        }

        return view('admin.lands.program_edit', compact('program'));
    }

public function update(Request $request, Lands_Indicator $indicator)
{
    $programExistsRule = Rule::exists('ppa', 'id')->where(function ($query) {
        $query->where('types_id', $this->getLandsTypeId())
            ->where('record_type_id', $this->getLandsRecordTypeIds()['PROGRAM']);
    ...
    if ($shouldCreateSnapshot) {
        $newIndicator = new Lands_Indicator();
...
public function destroyIndicator(Request $request, Lands_Indicator $indicator)
{
    $indicator->delete();
...
public function destroyPhysicalRow(Request $request)
{
    $validated = $request->validate([
        'row_id' => [
            'required',
            'integer',
            Rule::exists('ppa', 'id')->where(fn ($query) => $query->where('types_id', $this->getLandsTypeId())),
        ],
    ...
    try {
        $ppaRow = DB::table('ppa')
            ->where('id', $rowId)
            ->where('types_id', $this->getLandsTypeId())
            ->first();
        ...
        $deletedTargets = $this->deleteSectionRowsForPhysicalRows(Lands_Target::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);
        $deletedAccomplishments = $this->deleteSectionRowsForPhysicalRows(Lands_Accomplishment::class, $ppaIdsToDelete, $indicatorIds, $year, $officeIds);

        $indicatorNames = empty($indicatorIds)
            ? []
            : Lands_Indicator::query()
...
public function storeTargets(Request $request)
{
    ...
    $existingRows = Lands_Target::query()
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Lands_Target();
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Lands_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
    ...
    foreach ($entries as $entry) {
        ...
        $record = $existingByKey[$lookupKey] ?? new Lands_Accomplishment();
        ...
        $rawRemarks = array_key_exists('remarks', $entry)
            ? ($entry['remarks'] ?? null)
            : $this->decodeRemarksFromLandsrage($record->remarks ?? null);
        $record->remarks = $this->encodeRemarksForLandsrage($rawRemarks);
        $record->values = json_encode([
            'user_id' => $userId,
            'program_id' => $programId,
            'row_id' => $rowId,
...
private function formatSectionRecordForJs($row, array $meta = []): array
{
    return [
        ...
        'remarks' => $this->decodeRemarksFromLandsrage($row->remarks ?? null),
    ];
}

private function encodeRemarksForLandsrage($remarks): ?string
{
    if ($remarks === null) {
        return null;
    }
...
private function decodeRemarksFromLandsrage($raw): string
{
    if ($raw === null) {
        return '';
    }
...
private function resolveIndicatorTargetRowId(int $rowId, string $indicatorName = ''): int
{
    ...
    if ($normalizedIndicatorName !== '') {
        $currentIndicatorName = Lands_Indicator::query()
...
private function getIndicatorsGroupedByProgram(array $programIds): Collection
{
    ...
    $indicatorsById = Lands_Indicator::query()
...
private function getLandsPrograms(?int $programId = null, string $search = '', ?int $year = null): Collection
{
    $recordTypeIds = $this->getLandsRecordTypeIds();
    $typeId = $this->getLandsTypeId();
...
private function findLandsProgram(int $programId): ?object
{
    return $this->getLandsPrograms($programId)->first();
}
...
private function getLandsTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('Lands type is not configured.');
    }

    return (int) $typeId;
}

private function getLandsRecordTypeIds(): array
{
