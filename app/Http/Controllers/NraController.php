...
{
    public function index(Request $request, Nra_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Nra_Pap::query();
        ...
        $indicators = Nra_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Nra_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Nra_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Nra_Pap::query()
    ...
    public function destroyPap(Nra_Pap $program)
    {
        DB::beginTransaction();
        try {
            Nra_Target::where('program_id', $program->id)->delete();
            Nra_Accomplishment::where('program_id', $program->id)->delete();
            Nra_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:nra_pap,id',
        ...
        $indicator = Nra_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:nra_pap,id',
            'entries.*.indicator_id' => 'required|exists:nra_indicators,id',
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Nra_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
