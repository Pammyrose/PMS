...
class CobbController extends Controller
{
    ...
    {
        $allPrograms = $this->getCobbPrograms();
        ...
        $programs = $grouped;
        return view('admin.cobb.cobb', compact('programs', 'offices'));
    }
    ...
    public function save(Request $request)
    {
        ...
        $redirectRoute = $firstProgramId 
            ? route('admin.cobb_physical.physical', $firstProgramId)
            : route('admin.cobb_physical.physical');
    ...
    {
        return view('admin.cobb.indicator_create');
    }
    ...
    public function destroyPap($program)
    {
        ...
        try {
               ...
               foreach ($groupIds as $programId) {
                   $programRow = $this->findCobbProgram($programId);
                   if (!$programRow) {
                       continue;
                   }
                   ...
                   $this->deleteProgramSectionRows($programRow->id, Cobb_Target::query()->get());
                   $this->deleteProgramSectionRows($programRow->id, Cobb_Accomplishment::query()->get());
    ...
    {
        $program = $this->findCobbProgram((int) $program);
...
public function storeTargets(Request $request)
{
    ...
    $existingRows = Cobb_Target::query()
...
public function storeAccomplishments(Request $request)
{
    ...
    // Preload possible existing rows once, then match in-memory by year+office+program+indicator.
    $existingRows = Cobb_Accomplishment::query()
        ->whereIn('years', $years->all())
        ->where(function ($query) use ($nonNullOfficeIds, $hasNullOffice) {
            if ($nonNullOfficeIds->isNotEmpty()) {
                $query->whereIn('office_ids', $nonNullOfficeIds->all());
...
private function findCobbProgram(int $programId): ?object
{
    return $this->getCobbPrograms($programId)->first();
}
...
private function getCobbTypeId(): int
{
    ...
    if (!$typeId) {
        throw new \RuntimeException('COBB type is not configured.');
    }

    return (int) $typeId;
}

private function getCobbRecordTypeIds(): array
{
