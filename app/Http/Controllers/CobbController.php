...
{
    public function index(Request $request, Cobb_Pap $program = null)
    {
        $year = $request->query('year', now()->year);
        $office_id = $request->query('office_id', 1);
        $search = trim((string) $request->query('search', ''));

        $programsQuery = Cobb_Pap::query();
        ...
        $indicators = Cobb_Indicator::whereIn('program_id', $programIds)->get()->groupBy('program_id');
        ...
        $targets = Cobb_Target::where('year', $year)
            ->get()
        ...
        $accomplishments = Cobb_Accomplishment::where('year', $year)
            ->get()
            ->groupBy('indicator_id')
            ->map(function ($rows) {
                return $rows->mapWithKeys(function ($row) {
        ...
        $papSubactivities = Cobb_Pap::query()
    ...
    public function destroyPap(Cobb_Pap $program)
    {
        DB::beginTransaction();
        try {
            Cobb_Target::where('program_id', $program->id)->delete();
            Cobb_Accomplishment::where('program_id', $program->id)->delete();
            Cobb_Indicator::where('program_id', $program->id)->delete();
            $program->delete();
    ...
    public function storeIndicator(Request $request)
    {
        $baseValidated = $request->validate([
            ...
            'program_id' => 'required|exists:cobb_pap,id',
        ...
        $indicator = Cobb_Indicator::firstOrNew([
    ...
    public function storeTargets(Request $request)
    {
        $validated = $request->validate([
            ...
            'entries.*.program_id' => 'required|exists:cobb_pap,id',
            'entries.*.indicator_id' => 'required|exists:cobb_indicators,id',
    ...
    public function storeAccomplishments(Request $request)
    {
        ...
        foreach ($validated['entries'] as $entry) {
            $record = Cobb_Accomplishment::firstOrNew([
                'indicator_id' => $entry['indicator_id'],
                'year' => $entry['year'],
                'office_id' => $entry['office_id'] ?? null,
            ]);
