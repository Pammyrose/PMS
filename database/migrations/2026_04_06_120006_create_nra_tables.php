...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('nra_universe')) {
            Schema::create('nra_universe', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('nra_targets')) {
            Schema::create('nra_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('nra_accomplishments')) {
            Schema::create('nra_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyNraRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('nra_accomplishments');
        Schema::dropIfExists('nra_targets');
        Schema::dropIfExists('nra_universe');
        Schema::dropIfExists('nra');
    }

    private function copyNraRowsFromSharedTables(): void
    {
        $nraTypeId = DB::table('types')->where('code', 'NRA')->value('id');
        if (!$nraTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('nra_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $nraTypeId) {
                    continue;
                }

                DB::table('nra_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('nra_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('nra_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
