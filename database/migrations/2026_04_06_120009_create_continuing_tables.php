...
return new class extends Migration {
    ...
    {
        if (!Schema::hasTable('continuing')) {
            Schema::create('continuing', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('continuing_universe')) {
            Schema::create('continuing_universe', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('continuing_targets')) {
            Schema::create('continuing_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('continuing_accomplishments')) {
            Schema::create('continuing_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyContinuingRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('continuing_accomplishments');
        Schema::dropIfExists('continuing_targets');
        Schema::dropIfExists('continuing_universe');
        Schema::dropIfExists('continuing');
    }

    private function copyContinuingRowsFromSharedTables(): void
    {
        $continuingTypeId = DB::table('types')->where('code', 'CONTINUING')->value('id');
        if (!$continuingTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('continuing_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $continuingTypeId) {
                    continue;
                }

                DB::table('continuing_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('continuing_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('continuing_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
