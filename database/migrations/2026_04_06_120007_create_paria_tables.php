...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('paria_targets')) {
            Schema::create('paria_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('paria_accomplishments')) {
            Schema::create('paria_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyPariaRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('paria_accomplishments');
        Schema::dropIfExists('paria_targets');
        Schema::dropIfExists('paria_universe');
        Schema::dropIfExists('paria');
    }

    private function copyPariaRowsFromSharedTables(): void
    {
        $pariaTypeId = DB::table('types')->where('code', 'PARIA')->value('id');
        if (!$pariaTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('paria_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $pariaTypeId) {
                    continue;
                }

                DB::table('paria_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('paria_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('paria_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
