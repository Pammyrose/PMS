...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('soilcon_targets')) {
            Schema::create('soilcon_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('soilcon_accomplishments')) {
            Schema::create('soilcon_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copySoilconRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('soilcon_accomplishments');
        Schema::dropIfExists('soilcon_targets');
        Schema::dropIfExists('soilcon_universe');
        Schema::dropIfExists('soilcon');
    }

    private function copySoilconRowsFromSharedTables(): void
    {
        $soilconTypeId = DB::table('types')->where('code', 'Soilcon')->value('id');
        if (!$soilconTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('soilcon_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $soilconTypeId) {
                    continue;
                }

                DB::table('soilcon_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('soilcon_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('soilcon_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
