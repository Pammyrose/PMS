...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('pa_targets')) {
            Schema::create('pa_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('pa_accomplishments')) {
            Schema::create('pa_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyPaRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('pa_accomplishments');
        Schema::dropIfExists('pa_targets');
        Schema::dropIfExists('pa_universe');
        Schema::dropIfExists('pa');
    }

    private function copyPaRowsFromSharedTables(): void
    {
        $paTypeId = DB::table('types')->where('code', 'PA')->value('id');
        if (!$paTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('pa_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $paTypeId) {
                    continue;
                }

                DB::table('pa_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('pa_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('pa_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
