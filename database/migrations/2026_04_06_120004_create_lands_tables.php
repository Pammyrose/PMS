...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('lands_universe')) {
            Schema::create('lands_universe', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('lands_targets')) {
            Schema::create('lands_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('lands_accomplishments')) {
            Schema::create('lands_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyLandsRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('lands_accomplishments');
        Schema::dropIfExists('lands_targets');
        Schema::dropIfExists('lands_universe');
        Schema::dropIfExists('lands');
    }

    private function copyLandsRowsFromSharedTables(): void
    {
        $landsTypeId = DB::table('types')->where('code', 'Lands')->value('id');
        if (!$landsTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('lands_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $landsTypeId) {
                    continue;
                }

                DB::table('lands_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('lands_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('lands_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
