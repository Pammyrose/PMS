...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('sto_targets')) {
        ...
        if (!Schema::hasTable('sto_accomplishments')) {
        ...
        $this->copyStoRowsFromGassTables();
    }
    ...
    private function copyStoRowsFromGassTables(): void
    {
        $stoTypeId = DB::table('types')->where('code', 'STO')->value('id');
        if (!$stoTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('sto_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $stoTypeId) {
                    continue;
                }

                DB::table('sto_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('sto_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('sto_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
