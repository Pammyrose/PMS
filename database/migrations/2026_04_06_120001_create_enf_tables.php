...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (!Schema::hasTable('enf_universe')) {
            Schema::create('enf_universe', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('enf_targets')) {
            Schema::create('enf_targets', function (Blueprint $table) {
                $table->id();
        ...
        if (!Schema::hasTable('enf_accomplishments')) {
            Schema::create('enf_accomplishments', function (Blueprint $table) {
                $table->id();
                $table->foreignId('office_ids')->nullable()->constrained('offices')->nullOnDelete();
                $table->json('values')->nullable();
        ...
        $this->copyEnfRowsFromSharedTables();
    }

    public function down(): void
    {
        Schema::dropIfExists('enf_accomplishments');
        Schema::dropIfExists('enf_targets');
        Schema::dropIfExists('enf_universe');
        Schema::dropIfExists('enf');
    }

    private function copyEnfRowsFromSharedTables(): void
    {
        $enfTypeId = DB::table('types')->where('code', 'ENF')->value('id');
        if (!$enfTypeId) {
            return;
        }
        ...
        if (Schema::hasTable('gass_targets') && DB::table('enf_targets')->count() === 0) {
            foreach (DB::table('gass_targets')->get() as $row) {
                $values = json_decode($row->values ?? '[]', true);
                $programId = (int) ($values['program_id'] ?? 0);

                if (($programTypeById[$programId] ?? null) !== (int) $enfTypeId) {
                    continue;
                }

                DB::table('enf_targets')->insert([
        ...
        if (Schema::hasTable('gass_accomplishments') && DB::table('enf_accomplishments')->count() === 0) {
            foreach (DB::table('gass_accomplishments')->get() as $row) {
                ...
                DB::table('enf_accomplishments')->insert([
                    'office_ids' => $row->office_ids,
                    'values' => $row->values,
                    'remarks' => $row->remarks,
                    'years' => is_numeric($row->years ?? null) ? (int) $row->years : null,
